...
def download_video_from_url(url, output_dir, max_quality="720p"):
    ...
    if not YT_DLP_AVAILABLE:
        return (
            False,
            "yt-dlp not available. Please install it: pip install yt-dlp",
            None,
        )
    ...
    try:
        ...
        def progress_hook(d):
            if d["status"] == "finished":
                ...
                if os.path.exists(file_path) and not any(
                    temp in os.path.basename(file_path)
                    for temp in [".f", ".part", ".temp"]
                ):
        ...
        if not downloaded_files:
            ...
            for file in os.listdir(output_dir):
                ...
                if (
                    ...
                    and not any(temp in file for temp in [".f", ".part", ".temp"])
                ):
        ...
        if downloaded_files:
            ...
            return (
                True,
                f"Successfully downloaded video ({file_size} bytes)",
                final_path,
            )
        else:
...
def extract_frames_from_video(
    video_path, output_folder, frames_per_scene=2, scene_threshold=30.0
):
    """
    ...
    if not VIDEO_PROCESSING_AVAILABLE:
        print(
            "[WARNING] Scene detection not available, falling back to uniform extraction"
        )
        return extract_uniform_frames(
            video_path, output_folder, 20
    ...
    try:
            ...
            if scene_duration > 0:
                for j in range(frames_per_scene):
                    ...
                    if ret:
                        ...
                        print(
                            f"[VIDEO] Extracted frame {frame_count}/{frames_per_scene * len(scene_list)}: {frame_filename}"
                        )
        ...
        return (
            True,
            f"Successfully extracted {len(extracted_frames)} keyframes from {len(scene_list)} scenes",
            extracted_frames,
        )
...
def extract_uniform_frames(video_path, output_folder, num_frames=20):
    ...
    try:
        ...
        print(
            f"[VIDEO] Video has {total_frames} frames, {duration:.2f} seconds duration"
        )
        ...
        for i in range(0, total_frames, frame_interval):
            ...
            if ret:
                ...
                extracted_frames.append(frame_path)
                print(
                    f"[VIDEO] Extracted uniform frame {len(extracted_frames)}/{num_frames}: {frame_filename}"
                )
        ...
        return (
            True,
            f"Successfully extracted {len(extracted_frames)} frames uniformly",
            extracted_frames,
        )
...
def extract_frames_with_progress(
    urls_text, output_folder, frames_per_scene, scene_threshold, max_quality
):
    ...
    if invalid_urls:
        status_messages.append(
            f"⚠️ Skipping {len(invalid_urls)} invalid/unsupported URLs: {', '.join(invalid_urls)}"
        )
    ...
    try:
        ...
        for i, url in enumerate(valid_urls):
            status_messages.append(
                f"📥 Downloading video {i+1}/{len(valid_urls)}: {url}"
            )

            try:
                success, message, downloaded_file = download_video_from_url(
                    url, temp_download_dir, max_quality
                )

                if success and downloaded_file:
                    downloaded_files.append(downloaded_file)
                    status_messages.append(
                        f"✅ Downloaded: {os.path.basename(downloaded_file)}"
                    )
                else:
        ...
        status_messages.append(
            f"🎬 Successfully downloaded {len(downloaded_files)} videos. Starting frame extraction..."
        )
        ...
        for i, video_path in enumerate(downloaded_files):
            ...
            status_messages.append(
                f"🎬 Processing video {i+1}/{len(downloaded_files)}: {video_name}"
            )

            # Create subfolder for this video's frames
            video_output_folder = os.path.join(
                output_folder, os.path.splitext(video_name)[0]
            )
        ...
        status_messages.append(
            f"🎉 Processing complete! Successfully processed {successful_videos}/{len(downloaded_files)} videos"
        )
        ...
        try:
            ...
            status_messages.append(
                f"⚠️ Warning: Could not clean up temporary directory: {str(e)}"
            )

    return "\n".join(status_messages)


def process_local_videos_with_progress(
    input_folder, output_folder, frames_per_scene, scene_threshold
):
    ...
    for i, video_path in enumerate(video_files):
        video_name = os.path.basename(video_path)
        status_messages.append(
            f"🎬 Processing video {i+1}/{len(video_files)}: {video_name}"
        )
    ...
    status_messages.append(
        f"🎉 Processing complete! Successfully processed {successful_videos}/{len(video_files)} videos"
    )
    status_messages.append(f"📊 Total frames extracted: {total_extracted}")
    status_messages.append(f"💾 Frames saved to: {output_folder}")
